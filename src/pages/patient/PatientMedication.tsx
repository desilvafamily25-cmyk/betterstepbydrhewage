      ...
      <div className="space-y-5">
        ...
        {showForm ? (
          <div className="bg-white rounded-2xl border border-[#E7E5E1] p-5 shadow-sm space-y-4">
            ...
            {/* Brand picker */}
            <div>
              <label className="block text-xs font-semibold text-[#3C4346] mb-2 uppercase tracking-wide">Medication</label>
                  ...
                  <button
                    ...
                    className={`rounded-xl border px-3 py-3 text-left transition-all ${
                      form.name === config.brand
                        ? 'bg-[#1B3D34] border-[#1B3D34] text-white'
                        : 'bg-white border-[#E7E5E1] text-[#1B3D34]'
                    }`}
                  >
            ...
            <div>
              ...
              {isOther ? (
                ...
                <div className="rounded-xl border border-[#E7E5E1] bg-[#f7f8fa] px-4 py-3 text-sm text-[#1B3D34]">
                  {form.frequency}
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select className={inputClass} value={form.medicationDay} onChange={e => setForm(f => ({ ...f, medicationDay: e.target.value }))}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="date" className={inputClass} value={form.nextDoseDate} onChange={e => setForm(f => ({ ...f, nextDoseDate: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-[#3C4346] mb-1.5 uppercase tracking-wide">Days Remaining</label>
                <input type="number" className={inputClass} value={form.estimatedDaysRemaining} onChange={e => setForm(f => ({ ...f, estimatedDaysRemaining: parseInt(e.target.value) }))} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="date" className={inputClass} value={form.prescriptionReviewDate} onChange={e => setForm(f => ({ ...f, prescriptionReviewDate: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-[#3C4346] mb-1.5 uppercase tracking-wide">GP Review Due</label>
                <input type="date" className={inputClass} value={form.gpReviewDate} onChange={e => setForm(f => ({ ...f, gpReviewDate: e.target.value }))} />
              </div>
            </div>
